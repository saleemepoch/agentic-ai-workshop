...
These are the seven lessons I want every Teque engineer to walk away with. Each one came out of an actual mistake or rewrite during the build, and each maps to a real piece of code and an ADR they can read for the long version. Use them as discussion prompts when walking the team through the workshop.
...
4. **When the same trivial bug appears in multiple places, the fix is centralisation, not vigilance.** The `parse_llm_json` helper in [src/utils/llm_json.py](src/utils/llm_json.py) exists because we found the same code-fence-stripping bug in five places independently (reranker, score_match, extract_requirements, faithfulness judge, relevance judge). The first fix was inline in the reranker. Then it broke again somewhere else. Then again. Then we extracted the helper. Use this as a teachable moment about *when* to refactor vs when to leave duplication alone — the trigger isn't "two places", it's "the same mistake will keep happening because the surface area is bigger than my attention span." See ADR-009's "Two-Tier Parsing" section.

5. **Resilience patterns are not free — pick them based on the actual failure characteristics, not "more is always better".** The story to tell: I wrapped the Voyage AI client in retry-with-backoff to handle free-tier rate limits, then *reverted* it. Why? Voyage's free-tier rate window is ~60 seconds wide, so any backoff long enough to clear the window also gives the user a 30-second frozen frontend. The retry pattern was the wrong tool for *that specific failure mode*. The right fix was adding a payment method to Voyage. Use this to push back on "always add retries" thinking. See "Where the resilience patterns are NOT applied" in [ADR-010](docs/adrs/010-error-handling.md).

6. **Provider errors deserve their own translation layer.** When Anthropic returns "credit balance too low", the frontend shouldn't show a stack trace — it should show a banner with the actual fix the user needs to make. The [src/errors.py](src/errors.py) module turns provider exceptions into structured `{provider, type, message, user_action}` envelopes that the frontend renders consistently. Walk the team through the file: it's small enough to read in five minutes and captures real institutional knowledge about provider quirks (like the "wrong workspace" trap with Anthropic API keys). See [ADR-011](docs/adrs/011-provider-error-translation.md).
